...
class MHCA(nn.Module):
    ...
    def __init__(self, out_channels, head_dim):
        ...
        self.group_conv3x3 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1,
                                       padding=1, groups=out_channels // head_dim, bias=False)#3   96 32 3 3
    ...
    def forward(self, x):
        #aaa=self.group_conv3x3.weight.data.size()
...
class NextViT(nn.Module):
    def __init__(self, stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, attn_drop=0, drop=0, num_classes=1000,
        ...
        input_channel = stem_chs[-1]# 64
        features = []
        idx = 0
        dpr = [x.item() for x in torch.linspace(0, path_dropout, sum(depths))]  #stochastic depth decay rule   0-0.2 30 parts
        for stage_id in range(len(depths)):   #len(depths)==4
