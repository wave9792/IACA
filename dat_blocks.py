...
class DAttentionBaseline(nn.Module):
    ...
    def __init__(
        ...
        self.conv_offset = nn.Sequential(
            nn.Conv2d(self.nc, self.nc, kk, stride, kk // 2),
    ...
    def forward(self, x):
        ...
        q = self.proj_q(x)
       # q_off = einops.rearrange(q, 'b (g c) h w -> (b g) c h w', g=self.n_groups, c=self.n_group_channels)
        offset = self.conv_offset(q)  # B * g 2 Hg Wg
        ...
        displacement = (
                q_grid.reshape(B , H * W, 2).unsqueeze(2) - pos.reshape(B ,
                                                                                       n_sample,
                                                                                       2).unsqueeze(1)).mul(0.5)
        attn_bias = F.grid_sample(
        ...
        y = self.proj_drop(self.proj_out(out))
        y = self.conv(y)
        return y, pos.reshape(B,Hk, Wk, 2), reference.reshape(B,  Hk, Wk, 2)
